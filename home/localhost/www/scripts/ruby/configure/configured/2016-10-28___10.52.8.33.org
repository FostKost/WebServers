#-------------------------------------------------------------------------------
#                    DGS-3000-10TC Gigabit Ethernet Switch
#                                Configuration
#
...
config vlan default add tagged 9,10,2,3
create vlan 79 tag 9,10,2,3
config vlan 79 add tagged 9,10,2,3
create vlan 101 tag 101
config vlan 101 add tagged 9,10,2,3
create vlan 201 tag 201
config vlan 201 add tagged 9,10,2,3
create vlan 901 tag 901
config vlan 901 add tagged 9,10,2,3
create vlan 3101 tag 3101
config vlan 3101 add tagged 9,10,2,3
create vlan 4002 tag 4002
config vlan 4002 add tagged 9,10,2,3
create vlan 4003 tag 4003
config vlan 4003 add tagged 9,10,2,3
config vlan 4003 add untagged 1,4,5,6,7,8
disable qinq
disable gvrp
config gvrp nni_bpdu_addr dot1d
disable vlan_trunk
config port_vlan 10 gvrp_state disable ingress_checking enable acceptable_frame admit_all pvid 1

# IP

config ipif System ipaddress 10.52.8.33/255.255.255.192 vlan 101 state enable
disable autoconfig
...
config port_security ports 1,4,5,6,7,8 admin_state enable max_learning_addr 2 lock_address_mode deleteontimeout
config port_security ports 9,10,2,3 admin_state disable max_learning_addr 1 lock_address_mode deleteontimeout

# STORM

config traffic control 1,4,5,6,7,8 broadcast enable multicast enable unicast disable action drop threshold 128 countdown 0 time_interval 5
config traffic control 9,10,2,3 broadcast enable multicast enable unicast disable action drop threshold 4096 countdown 0 time_interval 5
...
config loopdetect ports 1,4,5,6,7,8 state enable
config loopdetect ports 9,10,2,3 state disable
config loopdetect trap none

# TRAF-SEGMENTATION

config traffic_segmentation all forward_list 9,10
...
config bandwidth_control 9,10,2,3 rx_rate no_limit tx_rate no_limit
...
config 802.1p default_priority 1-9 1
...
config lldp ports 9,10,2,3 admin_status tx_and_rx
config lldp ports 9,10,2,3 mgt_addr ipv4 10.52.8.33 enable
config lldp ports 1,4,5,6,7,8 admin_status rx_only
...
config access_profile profile_id 2 add access_id 45 ip vlan_id 4003 tcp dst_port 42 port all deny
config access_profile profile_id 2 add access_id 50 ip vlan_id 4003 tcp dst_port 135 port all deny
config access_profile profile_id 2 add access_id 55 ip vlan_id 4003 tcp dst_port 136 port all deny
config access_profile profile_id 2 add access_id 60 ip vlan_id 4003 tcp dst_port 137 port all deny
config access_profile profile_id 2 add access_id 65 ip vlan_id 4003 tcp dst_port 138 port all deny
config access_profile profile_id 2 add access_id 70 ip vlan_id 4003 tcp dst_port 139 port all deny
config access_profile profile_id 2 add access_id 75 ip vlan_id 4003 tcp dst_port 445 port all deny
config access_profile profile_id 2 add access_id 80 ip vlan_id 4003 tcp dst_port 1900 port all deny
...
config access_profile profile_id 3 add access_id 50 ip vlan_id 4003 udp dst_port 42 port all deny
config access_profile profile_id 3 add access_id 55 ip vlan_id 4003 udp dst_port 135 port all deny
config access_profile profile_id 3 add access_id 60 ip vlan_id 4003 udp dst_port 136 port all deny
config access_profile profile_id 3 add access_id 65 ip vlan_id 4003 udp dst_port 137 port all deny
config access_profile profile_id 3 add access_id 70 ip vlan_id 4003 udp dst_port 138 port all deny
config access_profile profile_id 3 add access_id 75 ip vlan_id 4003 udp dst_port 139 port all deny
config access_profile profile_id 3 add access_id 80 ip vlan_id 4003 udp dst_port 1900 port all deny
config access_profile profile_id 3 add access_id 85 ip vlan_id 4003 udp dst_port 5355 port all deny
config access_profile profile_id 3 add access_id 90 ip vlan_id 4003 destination_ip 224.0.0.0 mask 255.0.0.0 port all deny
...
config access_profile profile_id 3 add access_id 200 ip vlan_id 4002 udp dst_port 68 port all deny
config access_profile profile_id 3 add access_id 205 ip vlan_id 4003 udp dst_port 68 port all deny
config access_profile profile_id 3 add access_id 210 ip vlan_id 3101 udp dst_port 68 port all deny
...
config access_profile profile_id 4 add access_id 20 ethernet vlan_id 3101 port all deny
config access_profile profile_id 4 add access_id 30 ethernet vlan_id 4002 port all deny
config access_profile profile_id 3 add access_id auto_assign ip source_ip 92.50.143.178 mask 255.255.255.255 port all permit priority 7 replace_priority 
config access_profile profile_id 3 add access_id auto_assign ip destination_ip 92.50.143.178 mask 255.255.255.255 port all permit priority 7 replace_priority 
...
disable jumbo_frame
config ports 1-8 speed auto flow_control disable learning enable state enable mdix auto
config ports 9-10 medium_type copper speed auto flow_control disable learning enable state enable mdix auto
config ports 9-10 medium_type fiber speed auto flow_control disable learning enable state enable

# EEE

config eee ports 1-10 state disable
...
config qinq ports 1-10  missdrop disable outer_tpid 0x88A8
...
config stp mst_config_id name C8:D3:A3:43:3C:D0 revision_level 0
disable stp
...
config jwac ports 1-10 auth_mode host_based
config jwac ports 1-10 max_authenticating_host 50 aging_time 1440 idle_time infinite block_time 60
...
config lldp_med notification topo_change ports 1-10 state disable

# IGMP_SNOOPING

disable igmp_snooping
config igmp_snooping data_driven_learning max_learned_entry 128
config igmp_snooping vlan_name default fast_leave disable report_suppression enable proxy_reporting state disable source_ip 0.0.0.0 state disable
...
config mld_snooping vlan_name default fast_done disable report_suppression enable proxy_reporting state disable source_ip :: state disable
...
#-------------------------------------------------------------------
#             End of configuration file for DGS-3000-10TC
#-------------------------------------------------------------------