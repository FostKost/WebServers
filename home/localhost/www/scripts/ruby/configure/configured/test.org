...
ip access-list extended trusted-hosts
  permit ip 10.52..0 0.0.0.255 any-destination
  permit ip host-source 79.140.16.7 any-destination
  permit ip host-source 79.140.16.8 any-destination
  permit ip host-source 81.30.199.70 any-destination
...
policy-map 1
 ...
 exit
!
vacl ip access-group permit-and-block-fiz in traffic-statistic vlan 
vacl ip access-group permit-and-block in traffic-statistic vlan ;
!
...
!
interface 
 ip address 10.52.8.5 255.255.255.192
!
ip default-gateway 
!
sntp polltime 600
...
no login
!
isolate-port group Down switchport interface Ethernet1/
